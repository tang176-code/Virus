...
Purpose and use: this demo shows the random genomic-position null. It samples nuclear genomic windows with the same length as human mtDNA, so it controls scanned nucleotide length and random genomic position. It does not match GC, CpG, dinucleotide, or other sequence-composition features.
...
# Nuclear genome used for random nuclear-window sampling, soft-masked GRCh38 primary assembly
...
- Random-window standard: positions are drawn randomly from the nuclear chromosomes, weighted by chromosome length, with high-`N` windows excluded
...
```bash
python run_mtDNA_length_random_nuclear_window_demo.py \
...
- `mtDNA_length_random_nuclear_windows_demo.csv`
- `mtDNA_length_random_nuclear_windows_demo.fa`
- `mtDNA_length_random_nuclear_windows_demo_summary.txt`

Columns in the CSV:

- `random_order`: order in which windows were accepted
...
- `absolute_gc_difference`: absolute GC difference between the nuclear window and mtDNA; reported for reference only, not used for matching
